...
int mcx_raster_subgrid(cJSON *obj, Grid3D *g){
    ...
    for(k=0;k<g->dim->z;k++){
      if(k<O[2] || k>O[2]+S[2]) continue;
      for(j=0;j<g->dim->y;j++){
        if(j<O[1] || j>O[1]+S[1]) continue;
    	for(i=0;i<g->dim->x;i++){
            if(i<O[0] || i>O[0]+S[0]) continue;
            (*(g->vol))[g->rowmajor? i*dimyz+j*g->dim->z+k : k*dimxy+j*g->dim->x+i]=tag;
        }
      }
    }
...
int mcx_raster_box(cJSON *obj, Grid3D *g){
    ...
    for(k=0;k<g->dim->z;k++){
      dz=(k+0.5f);
      if(dz<O[2] || dz>O[2]+S[2]) continue;
      for(j=0;j<g->dim->y;j++){
    	dy=(j+0.5f);
        if(dy<O[1] || dy>O[1]+S[1]) continue;
    	for(i=0;i<g->dim->x;i++){
            dx=(i+0.5f);
            if(dx<O[0] || dx>O[0]+S[0]) continue;
            (*(g->vol))[g->rowmajor? i*dimyz+j*g->dim->z+k : k*dimxy+j*g->dim->x+i]=tag;
        }
      }
    }
